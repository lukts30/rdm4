...
pub struct TransformContext {
    ...
    pub(crate) transpose_inv_transform_mat3:
...
pub fn p3f(position: Point<f32, 3>) -> P3f {
    P3f {
        data: [1.0 * position[0], 1.0 * position[1], 1.0 * position[2]],
    }
}
...
pub fn n3f(vec_normal: Vector3<f32>) -> N3f {
    N3f {
        data: [vec_normal.x, vec_normal.y, vec_normal.z],
    }
}
...
pub fn g3f(vec_tangent: Vector3<f32>) -> G3f {
    G3f {
        data: [vec_tangent.x, vec_tangent.y, vec_tangent.z],
    }
}
...
pub fn t2h(uv: [f32; 2]) -> T2h {
    T2h {
        data: [f16::from_f32(uv[0]), f16::from_f32(uv[1])],
    }
}
